...
let comboCurrent = document.getElementById("comboCurrent");
const modsImgs = {
  ez: "./static/easy.png",
...
socket.onmessage = (event) => {
  ...
  if (data.gameplay.combo.current != "") {
    comboCurrent.innerHTML = data.gameplay.combo.current;
  } else {
    const currentState = data.menu.state;
    // console.log("menu.state", data.menu.state);
    // 0 = , 1 = , 5 = , 7 = results
    const passiveStates = [0, 1, 5, 7];
    if (passiveStates.includes(currentState)) {
      comboCurrent.innerHTML = data.menu.bm.stats.maxCombo;
    } else {
      comboCurrent.innerHTML = "0";
    }
  }
